...
if game == 'cons':
    ...
    checkpoints = [f'logs/soccer_uncons_effort_square/t_{i}/checkpoints_dir/model_final.pth'
                   for i in range(1, 11)]
...
if __name__ == "__main__":
    ...
    data = {'states': np.vstack(states),
            'values': values}
    scio.savemat('states_pierre_square.mat', data)
    ...
    U = np.vstack(U)
    D = np.vstack(D)
    axs[2].plot(np.linspace(0, 0.9, 10), U[:, 0], label='$u_x$')
    axs[2].plot(np.linspace(0, 0.9, 10), D[:, 0], label='$d_x$')
    axs[2].plot(np.linspace(0, 0.9, 10), U[:, 1], '-.', label='$u_y$')
    axs[2].plot(np.linspace(0, 0.9, 10), D[:, 1], '--', label='$d_y$')
    ...
    axs[1].plot(np.linspace(0, 1, 11), p_t, linewidth=2)
