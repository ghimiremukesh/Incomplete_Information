    ...
    for i in range(num_games):
        ...
        for j in range(1, Time.shape[0] + 1):
            ...
            for m in range(2 ** (j - 1)):
                X_nn = np.vstack((d_list[j - 1][m], v_list[j - 1][m], p_list[j - 1][m]))

                _, _, V = value_action(X_nn, t_nn, model)

                u1, u2, p_t = optimization(X_nn, t_nn, dt, model, type_i)
                u1_list[j - 1].append(u1)
                u2_list[j - 1].append(u2)
                V_list[j - 1].append(V)
                ...
                for n in range(2):
                    if j == Time.shape[0]:
                        ...
                        d, v = dynamic(X_nn, dt, (u1_list[j - 1][m][n], u2_list[j - 1][m][n]))
                        d_list[j].append(d)
                        v_list[j].append(v)
                        p_list[j].append(p_t[n])
            print(j)
