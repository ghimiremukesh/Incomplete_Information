...
import dataio, utils, training, model, icnn_pytorch_adaptive as icnn_pytorch, modules
...
# ckpt_path = '../experiment_scripts/logs/t_0/checkpoints/model_final.pth'
# ckpt_path = '../experiment_scripts/logs/test_non_cvx/t_0/checkpoints/model_current.pth'
# ckpt_path = '../experiment_scripts/logs/test_non_cvx/t_1/model_final.pth'
# ckpt_path = '../experiment_scripts/logs/final_train_re_sgd/t_2/checkpoints_dir/model_final.pth'
# ckpt_path = '../experiment_scripts/logs/adaptive_old/t_10/checkpoints_dir/model_final.pth'
# c_inter = '../experiment_scripts/logs/final_train_re_sgd/t_1/checkpoints_dir/model_final.pth'
# c_inter = '../experiment_scripts/logs/adaptive_old/t_9/checkpoints_dir/model_final.pth'
#
ckpt_path = '../logs/train_new_case_follow_zero_shallow/t_1/checkpoints_dir/model_final.pth'
c_inter = '../logs/train_new_case_follow_zero_shallow/t_9/checkpoints_dir/model_final.pth'

# ckpt_path = '../logs/train_new_case_funnel_scale/t_1/checkpoints_dir/model_final.pth'
# c_inter = '../logs/train_new_case_funnel_scale/t_8/checkpoints_dir/model_final.pth'

# from cluster
#
# ckpt_path = '../experiment_scripts/logs/train_new_case_funnel_adaptive_scale/t_10/checkpoints_dir/model_final.pth'
# c_inter = '../experiment_scripts/logs/train_new_case_funnel_adaptive_scale/t_9/checkpoints_dir/model_final.pth'

# ckpt_path = '../logs/train_new_case_funnel_adap_one/t_10/checkpoints_dir/model_final.pth'
# c_inter = '../logs/train_new_case_funnel_adap_one/t_9/checkpoints_dir/model_final.pth'

activation = 'relu'

# initialize the model

model = icnn_pytorch.SingleBVPNet(in_features=6, out_features=1, type=activation, mode='mlp', hidden_features=128,
                                  num_hidden_layers=2)

model_inter = icnn_pytorch.SingleBVPNet(in_features=6, out_features=1, type=activation, mode='mlp', hidden_features=128,
                                  num_hidden_layers=2)
...
# x[:, 1] = torch.linspace(-0.1, 0.1, num_points)
x[:, 2] = 0
...
# p * np.maximum(0, x) + (1 - p) * np.maximum(0, -x)

# V_true = - P * np.maximum(0, dx.detach().cpu().numpy()) - (1 - P) * np.maximum(0, -dx.detach().cpu().numpy())
# V_true = -P * np.maximum(0, X) - (1 - P) * np.maximum(0, -X)
...
x_next = torch.from_numpy(utils.dynamics_4d_follow_zero(x_prev, umax, dmax, DT=0.1))
x_next = torch.cat((t_next * torch.ones((x_next.shape[0], 1)), x_next), dim=1)
x_next = np.array([utils.make_payoff_zero(x_next[i, :], x_next[i + 1, :], x_next[i + 2, :])
                   for i in range(0, len(x_next), 3)]).reshape(-1, 5)  # change this to vecotrized op.
