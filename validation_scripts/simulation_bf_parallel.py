...
def solve_optimization(params):
    (lam, p1, p2, X_nn, t_nn, dt, model) = params
    var = np.array([lam, p1, p2])
    constrain = constraint(var, X_nn)

    if constrain <= 5e-3:
        sol = objective(var, X_nn, t_nn, dt, model)
        opt_sol = np.array([sol])
        opt_x = np.array([lam, p1, p2])

        return (opt_sol, opt_x)
...
def optimization(X_nn, t_nn, dt, model):
    ...
    for (lam, p1, p2) in product(Lam, P1, P2):
        params.append((lam, p1, p2, X_nn, t_nn, dt, model))
    res_list = list(p.imap(solve_optimization, params))
    p.close()
    p.join()

    for res in res_list:
        opt_sol['sol'].append(res[0])
        opt_sol['opt_x'].append(res[0])
