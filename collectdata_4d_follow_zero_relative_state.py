...
def make_payoff(x, y, z):
    ...
    X_minus = (x_minus, xv_minus, yv_minus)
    X_zero = (x_zero, xv_zero, yv_zero)
    X_plus = (x_plus, xv_zero, xv_minus)
    Y_minus = (yv_minus, yv_zero, yv_plus)
    Y_zero = (yv_zero)
    Y_plus = (yv_plus)
...
if __name__ == "__main__":
    num_points = 4000
    extra_points = 2000 # sample around 0
    t = opt.time
    t_step = int(t * 100) // 10
    dt = 0.1
    umax = 1
    ...
    for _ in range(9):
        ...
        # states.append((states[-1])/1.1)


    # try relative position, i.e. \delx, v1, v2
    # x = torch.zeros(num_points, 4).uniform_(-states[t_step-1], states[t_step-1])  # x1, v1, x2, v2
    x = torch.zeros(num_points, 1).uniform_(-states[t_step-1], states[t_step-1])


    # x_extra_1 = torch.zeros(extra_points, 4).uniform_(-0.1, 0.1)
    x_extra_1 = torch.zeros(extra_points, 1).uniform_(-0.01, 0.01)
    x_extra_2 = torch.zeros(extra_points, 1).uniform_(-0.001, 0.001)
    x_extra_3 = torch.zeros(extra_points, 1).uniform_(-0.0025, 0.0025)

    # now sample velocities:
    v = torch.zeros(num_points + 3 * extra_points, 2).uniform_(-1, 1)

    x = torch.cat((x, x_extra_1, x_extra_2, x_extra_3), dim=0)
    ...
    if t == 0.1:
        ...
        for p_each in ps:
            ...
            v_next = utils.final_value_minmax((x_next_p[:, 1]), x_next_p[:, -1]).numpy()
        ...
        scio.savemat(os.path.join(save_root, f'train_data_t_{t}.mat'), gt)

    else:
        t_next = t - dt

        load_dir = os.path.join(logging_root, f'train_new_case_follow_zero_relative/t_{t_step - 1}/')
        start_epoch = 4 #14 #14  # num_epoch - 1 from training

        val_model = icnn_pytorch.SingleBVPNet(in_features=5, out_features=1, type=activation, mode='mlp',
