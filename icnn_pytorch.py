...
class FCBlock(nn.Module):
    ...
    def forward(self, coords, params=None, **kwargs):
        ...
        # prevZ, prevU = None, coords[..., :-1]
        # y_input = coords[..., -1:]
        #
        # for i, sz in enumerate([self.hidden_features] * (self.num_hidden_layers) + [1]):
        #     if sz != 1:
        #         u = self.net_u[i](prevU)
        #
        #     z_add = []
        #
        #     if prevZ is not None:
        #         z_add.append(self.net_z_zu[i-1](torch.mul(prevZ, self.net_zu_u[i-1](prevU))))
        #
        #     z_add.append(self.net_z_yu[i](torch.mul(y_input, self.net_yu_u[i](prevU))))
        #
        #     z_add.append(self.net_z_u[i](prevU))
        #
        #     z = sum(z_add)
        #     if sz!= 1:
        #         z = self.nl(z)
