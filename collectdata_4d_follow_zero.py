...
import os
import icnn_pytorch as icnn_pytorch
...
if __name__ == "__main__":
    ...
    num_ps = 100
    logging_root = 'logs/'
    save_root = 'new_case_data_follow_zero_shallow/'

    if not os.path.exists(save_root):
        os.makedirs(save_root)

    # load_dir = os.path.join(logging_root, f't_0/')
    # start_epoch = 19
    #
    ...
    # x = torch.zeros(num_points, 4).uniform_(-up, up)  # x1, v1, x2, v2

    # if t == 1:
    #     x = torch.zeros(num_points, 4).uniform_(-0.01, 0.01)

    # x_extra_1 = torch.zeros(extra_points, 4).uniform_(-0.1, 0.1)
    x_extra_2 = torch.zeros(extra_points, 4).uniform_(-0.1, 0.1)
    x_extra_3 = torch.zeros(extra_points, 4).uniform_(-0.05, 0.05)
    x_extra_4 = torch.zeros(extra_points, 4).uniform_(-0.01, 0.01)
    x = torch.cat((x, x_extra_2, x_extra_3, x_extra_4), dim=0)



    # p = torch.zeros(num_points + 3 * extra_points, 1).uniform_(0, 1)

    # instead of uniformly sampling p, for each x get data on all p
    # ps = torch.linspace(0, 1, 100)
    # p = ps.repeat([len(x), 1]).reshape(-1, 1)
    # x = torch.vstack([x[i].repeat([NUM_PS, 1]) for i in range(len(x))])
    # p = torch.tensor([ps for _ in x], dtype=torch.float32)  # don't really need this
    ...
    if t == 0.1:
        ...
        load_dir = os.path.join(logging_root, f'train_new_case_follow_zero_shallow/t_{t_step - 1}/')
        start_epoch = 5  # num_epoch - 1 from training
